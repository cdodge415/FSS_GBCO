...
# may want to filter for just dqi_cd == R and/or A
class(SC$dqi_cd)
SC$dqi_cd <- as.factor(SC$dqi_cd)
levels(SC$dqi_cd)
SC_A <- SC[which(SC$dqi_cd == "A"),]
SC_R <- SC[which(SC$dqi_cd == "R"),]
SC_AR <- rbind(SC_A, SC_R)
rm(SC_A, SC_R)
saveRDS(SC, "USGS_SC_qw_dqiAR.rds")

colnames(SC)
SC <- select(SC_AR, c("site_no", "sample_dt", "sample_tm", "result_va"))
colnames(SC) <- c("SiteID", "Date","Time", "Specific.conductance")
